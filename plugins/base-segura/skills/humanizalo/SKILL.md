...
version: 1.0.0
description: |
  Hace que un texto escrito por IA suene a persona: detecta y elimina 40 señas de IA
  (vocabulario, estructura, formato, relleno y tics de chatbot), le inyecta voz, se
  califica en 6 dimensiones y no entrega hasta pasar 42/60 o agotar 3 pasadas.
  Usala cuando el usuario diga "esto suena a IA", "se nota que lo escribió una máquina",
  "humanizá esto", "que suene a mí", "sacale el tono robot", "dale voz", "revisá el tono",
  o cuando haya que escribir o revisar un correo, una propuesta, un texto de web, un
  anuncio o un posteo que va a leer otra persona. Funciona en español y en inglés
  (las señas en español están en references/espanol.md).
  Detects and eliminates 40 AI writing tells; use when editing or reviewing any text
  to make it sound unmistakably human.
license: MIT
sync: si
origen: "Hainrixz/humanizalo (MIT, github.com/Hainrixz/humanizalo), vía tododeia.com/community/modo-directo-claude. Vendorizada 2026-09-22: description con gatillos en español, allowed-tools recortado a lo que la skill usa, references/espanol.md agregado (propio). Auditada: PASS."
allowed-tools:
  - Read
  - Edit
  - AskUserQuestion
...
Las categorías C (estructura), D (formato) y E (tics de chatbot) aplican igual en cualquier idioma.
La categoría B (vocabulario) de abajo lista palabras inglesas: para textos en español usá la lista
equivalente de `references/espanol.md`, que además trae las señas propias del español
(gerundios de relleno, «el mismo/la misma», diminutivos de cortesía, mayúsculas de respeto).
Entregá el texto en el idioma en que te lo dieron.
